...
type PrePrepareMsg struct {
    ...
    SequenceNum uint64  // 시퀀스 번호 (블록 높이)
    Digest      []byte  // 블록 해시
    BlockData   []byte  // 블록 데이터 (직렬화된 바이트)
    PrimaryId   string  // 리더 노드 ID
}
```
...
type BlockHeader struct {
    Height    uint64                 // 블록 높이
    Timestamp *timestamppb.Timestamp // 타임스탬프
    PrevHash  []byte                 // 이전 블록 해시
    TxHash    []byte                 // 트랜잭션 머클 루트
    StateHash []byte                 // 상태 해시
...
type ExecutionResult struct {
    ...
    ConsensusParamUpdates *abci.ConsensusParams  // 합의 파라미터 업데이트
    AppHash               []byte                 // 앱 해시
...
type Mempool struct {
    ...
    // 트랜잭션 저장소
    txStore     map[string]*Tx      // ID → Tx
    senderIndex map[string][]*Tx    // Sender → Tx 목록
    txQueue     *priorityQueue      // 우선순위 큐
...
type Config struct {
    MaxTxs        int            // 최대 트랜잭션 수 (5000)
    MaxBytes      int64          // 최대 총 바이트 (1GB)
    MaxTxBytes    int64          // 단일 트랜잭션 최대 (1MB)
    TTL           time.Duration  // 트랜잭션 수명 (1시간)
    CacheSize     int            // 캐시 크기 (10000)
    CheckTxAsync  bool           // 비동기 CheckTx
}
```
...
type GRPCTransport struct {
    ...
    // 피어 연결
    peers map[string]*peerConn  // peerID → 연결

    // 메시지 핸들러
    messageHandler func(peerID string, msg *pbftv1.PBFTMessage)
...
type peerConn struct {
    ...
    stream  pbftv1.PBFTService_StreamMessagesClient
}

type GRPCTransportConfig struct {
    ListenAddr     string         // "0.0.0.0:26656"
    MaxMessageSize int            // 최대 메시지 크기
    DialTimeout    time.Duration  // 연결 타임아웃
    KeepAlive      time.Duration  // Keep-alive 간격
}
```

#### Transport (network/transport.go) - TCP 기반
```go
type Peer struct {
    ID      string
    Addr    string
...
type Transport struct {
    ...
    // 메시지 처리
    incomingCh chan *Message  // 수신 메시지 채널
...
type Metrics struct {
    // Prometheus 메트릭
    consensusRounds    prometheus.Counter    // 총 합의 라운드
    consensusDuration  prometheus.Histogram  // 합의 소요 시간
    blockHeight        prometheus.Gauge      // 현재 블록 높이
    messagesSent       prometheus.Counter    // 전송 메시지 수
    messagesReceived   prometheus.Counter    // 수신 메시지 수
    tps                prometheus.Gauge      // 초당 트랜잭션
    mempoolSize        prometheus.Gauge      // 멤풀 크기
    peerCount          prometheus.Gauge      // 피어 수
    viewChanges        prometheus.Counter    // 뷰 변경 횟수
}
...
type Node struct {
    ...
    transport *transport.GRPCTransport  // P2P 전송
    mempool   *mempool.Mempool   // 트랜잭션 풀
    reactor   *mempool.Reactor   // 멤풀 네트워크 리액터
    abci      *pbft.ABCIAdapter  // ABCI 어댑터
...
type Config struct {
    ...
    Peers      []string              // 피어 목록
    Validators []*pbft.ValidatorInfo // 검증자 목록
